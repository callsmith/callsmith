...
public sealed class BrunoCollectionServiceTests : IDisposable
{
    ...
    // ─────────────────────────────────────────────────────────────────────────
    //  ResolveRequestFilePathAsync
    // ─────────────────────────────────────────────────────────────────────────

    [Fact]
    public async Task ResolveRequestFilePathAsync_FlatFileName_ReturnsFilePath()
    {
        // File named "login.bru" with meta.name "Log In" — should resolve by file name.
        WriteFile("login.bru", BruFile("Log In", "post", "https://api.example.com/login", seq: 1));

        var result = await _sut.ResolveRequestFilePathAsync(_root, "login");

        Assert.Equal(Path.Combine(_root, "login.bru"), result);
    }

    [Fact]
    public async Task ResolveRequestFilePathAsync_FlatFileName_FileNameDiffersFromMetaName_PrefersFileName()
    {
        // meta.name = "Get Items", file = "get-items.bru" — query by file name wins.
        WriteFile("get-items.bru", BruFile("Get Items", "get", "https://api.example.com/items", seq: 1));

        var byFileName = await _sut.ResolveRequestFilePathAsync(_root, "get-items");
        var byMetaName = await _sut.ResolveRequestFilePathAsync(_root, "Get Items");

        Assert.Equal(Path.Combine(_root, "get-items.bru"), byFileName);
        Assert.Equal(Path.Combine(_root, "get-items.bru"), byMetaName);
    }

    [Fact]
    public async Task ResolveRequestFilePathAsync_PathQualified_NavigatesByDirectoryAndFileName()
    {
        // Structure: auth/ (dir) → login.bru (meta.name "Sign In")
        // Should resolve "auth/login" by directory name + file name, not meta.names.
        Directory.CreateDirectory(Path.Combine(_root, "auth"));
        WriteFile("auth/login.bru", BruFile("Sign In", "post", "https://api.example.com/login", seq: 1));

        var result = await _sut.ResolveRequestFilePathAsync(_root, "auth/login");

        Assert.Equal(Path.Combine(_root, "auth", "login.bru"), result);
    }

    [Fact]
    public async Task ResolveRequestFilePathAsync_PathQualified_DeeplyNested_ReturnsCorrectPath()
    {
        Directory.CreateDirectory(Path.Combine(_root, "users", "profile"));
        WriteFile("users/profile/update.bru",
            BruFile("Update Profile", "put", "https://api.example.com/users/profile", seq: 1));

        var result = await _sut.ResolveRequestFilePathAsync(_root, "users/profile/update");

        Assert.Equal(Path.Combine(_root, "users", "profile", "update.bru"), result);
    }

    [Fact]
    public async Task ResolveRequestFilePathAsync_FlatFileName_NotFound_ReturnsNull()
    {
        WriteFile("login.bru", BruFile("Login", "post", "https://api.example.com/login", seq: 1));

        var result = await _sut.ResolveRequestFilePathAsync(_root, "nonexistent");

        Assert.Null(result);
    }

    [Fact]
    public async Task ResolveRequestFilePathAsync_FlatFileName_SearchesSubfolders()
    {
        // Request is inside a subfolder — flat name lookup should find it.
        Directory.CreateDirectory(Path.Combine(_root, "auth"));
        WriteFile("auth/login.bru", BruFile("Login", "post", "https://api.example.com/login", seq: 1));
    ...
    [Fact]
    public async Task ResolveRequestFilePathAsync_FallsBackToMetaName_WhenFileNameNotFound()
    {
        // If no file named "sign-in.bru" exists but a file with meta.name "sign-in" does,
        // the meta.name fallback should find it.
        WriteFile("login.bru", BruFile("sign-in", "post", "https://api.example.com/login", seq: 1));

        var result = await _sut.ResolveRequestFilePathAsync(_root, "sign-in");
