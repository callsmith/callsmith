...
public class RequestAssemblyServiceTests
{
    ...
    [InlineData("content-type")]
    [InlineData("CONTENT-TYPE")]
    public async Task AssembleAsync_ExplicitContentTypeHeader_OverridesAutoGenerated(string headerKey)
    {
        var globalEnv = new EnvironmentModel { FilePath = "", EnvironmentId = Guid.NewGuid(), Name = "Global", Variables = [] };
        var mergedEnv = new ResolvedEnvironment { Variables = new Dictionary<string, string>() };
        _mergeServiceMock.MergeAsync(Arg.Any<string>(), Arg.Any<EnvironmentModel>(), Arg.Any<EnvironmentModel>(), Arg.Any<bool>(), Arg.Any<CancellationToken>())
        ...
        var input = new RequestAssemblyInput
        {
            ...
            Headers = [new KeyValuePair<string, string>(headerKey, "application/vnd.custom+json")],
            PathParams = [],
            QueryParams = [],
            BodyType = CollectionRequest.BodyTypes.Json,
            BodyText = """{"key":"value"}""",
        ...
        // The user's explicit Content-Type must win over the auto-generated one.
        assembled.RequestModel.ContentType.Should().Be("application/vnd.custom+json");
    }
    ...
    [InlineData("content-type")]
    [InlineData("CONTENT-TYPE")]
    public async Task AssembleAsync_ExplicitContentTypeHeader_IsNotDuplicatedInAutoAppliedHeaders(string headerKey)
    {
        var globalEnv = new EnvironmentModel { FilePath = "", EnvironmentId = Guid.NewGuid(), Name = "Global", Variables = [] };
        var mergedEnv = new ResolvedEnvironment { Variables = new Dictionary<string, string>() };
        _mergeServiceMock.MergeAsync(Arg.Any<string>(), Arg.Any<EnvironmentModel>(), Arg.Any<EnvironmentModel>(), Arg.Any<bool>(), Arg.Any<CancellationToken>())
        ...
        // When user supplies Content-Type, no auto-applied Content-Type should be emitted.
        assembled.AutoAppliedHeaders.Should().NotContain(h =>
            h.Key.Equals("Content-Type", StringComparison.OrdinalIgnoreCase));
    }

    [Fact]
    public async Task AssembleAsync_NoExplicitContentTypeHeader_EmitsAutoAppliedContentType()
    {
        var globalEnv = new EnvironmentModel { FilePath = "", EnvironmentId = Guid.NewGuid(), Name = "Global", Variables = [] };
        var mergedEnv = new ResolvedEnvironment { Variables = new Dictionary<string, string>() };
        _mergeServiceMock.MergeAsync(Arg.Any<string>(), Arg.Any<EnvironmentModel>(), Arg.Any<EnvironmentModel>(), Arg.Any<bool>(), Arg.Any<CancellationToken>())
        ...
        // Without a user-specified Content-Type, the auto-applied header must be present.
        assembled.AutoAppliedHeaders.Should().Contain(h =>
            h.Key.Equals("Content-Type", StringComparison.OrdinalIgnoreCase) &&
