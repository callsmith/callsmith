...
public sealed class HistoryRepository : IHistoryService
{
    ...
    // Creates a DbContext pointed at the specified database path.
    ...
    public async Task RecordAsync(HistoryEntry entry, CancellationToken ct = default)
    {
        ...
        await EnsureHistorySchemaAsync(db, dbPath, ct);
    ...
    /// <inheritdoc/>
    public ValueTask<HistoryEntry> RevealSensitiveFieldsAsync(
        HistoryEntry entry,
        CancellationToken ct = default)
    {
        ArgumentNullException.ThrowIfNull(entry);
        ...
        return ValueTask.FromResult(entry with { VariableBindings = revealedBindings });
    }
    ...
    private async Task EnsureHistorySchemaAsync(CallsmithDbContext db, string dbPath, CancellationToken ct)
    {
