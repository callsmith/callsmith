...
// ─────────────────────────────────────────────────────────────────────────────
// SyntaxEditorCompletionHandler — per-SyntaxEditor overlay controller
// ─────────────────────────────────────────────────────────────────────────────

/// <summary>
/// Manages the autocomplete overlay lifecycle for a single <see cref="SyntaxEditor"/>.
/// Works like <see cref="CompletionHandler"/> but uses AvaloniaEdit's document/caret API.
/// </summary>
internal sealed class SyntaxEditorCompletionHandler
{
    ...
    private void OnTextChanged(object? sender, EventArgs _)
    {
        if (_suppressChange) return;
    ...
    private void OpenPanel(List<EnvVarSuggestion> items)
    {
        ...
        // Position below the editor (using caret visual position if available, else bottom-left)
        var pos = GetCaretScreenPosition(overlay) ?? _editor.TranslatePoint(new Point(0, _editor.Bounds.Height), overlay);
        if (pos.HasValue)
        {
            Canvas.SetLeft(_overlayPanel!, pos.Value.X);
            Canvas.SetTop(_overlayPanel!, pos.Value.Y);
        ...
        _overlayPanel!.Width = Math.Max(180, _editor.Bounds.Width);
        _overlayPanel.IsVisible = true;
        _isVisible = true;
    }

    /// <summary>
    /// Tries to get the screen position just below the current caret in the editor.
    /// Falls back to null if the position cannot be determined.
    /// </summary>
    private Point? GetCaretScreenPosition(Visual overlay)
    {
        try
        {
            ...
            // GetVisualPosition returns a position relative to the TextView.
            var visualPos = tv.GetVisualPosition(
                _editor.TextArea.Caret.Position,
                VisualYPosition.LineBottom);

            // Translate from TextView coordinates to the overlay layer.
            return tv.TranslatePoint(visualPos, overlay);
        }
        catch
        {
            // Visual-position lookup can fail if the layout hasn't been measured yet or the
            // caret is temporarily out of range. Fall back to positioning below the editor.
            return null;
        }
    }
    ...
    private void CommitSelection()
    {
        ...
        var token = $"{{{{{chosen.Name}}}}}";   // {{name}}
        var replaceLength = caretOffset - trigger.StartIndex;

        _suppressChange = true;
        try
        {
            _editor.Document.Replace(trigger.StartIndex, replaceLength, token);
